...
expect_silent(write_df(list(df1, df2), tmp, sep = ";"))
expect_equal(readLines(tmp), c("a;b", "1;x", "2;y", "3;z", "", "a;b", "4;p", "5;q", "6;r"))
write_df(list(df1), tmp)
expect_equal(length(readLines(tmp)), 4L)
write_df(list(df2), tmp, overwrite = FALSE)
expect_equal(length(readLines(tmp)), 8L)
