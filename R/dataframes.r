...
#' Write Dataframes
#'
#' Write a list of rectangular dataframes to a single csv keeping column names, separating dataframes with a blank line.
#'
...
#' @param overwrite Boolean value, if true will always overwrite file even if it exists, otherwise appends to it.
#' @param ... Additional options passed to [data.table::fwrite()].
#' @export
write_df = function(list, file, overwrite = TRUE, ...) {
  for (i in seq_along(list)) {
    ...
    data.table::fwrite(list[[i]], file, append = i > 1L || !overwrite, col.names = TRUE, ...)
  }
}
